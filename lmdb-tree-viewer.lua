...
-- Usage: mdb_dump /path/to/lmdb | ./lmdb-tree-viewer [--summary|--parser]
...
local function show_summary(keys)
    print("LMDB Database Structure Summary")
    print("===============================")
    print()
...
local function show_parser(keys)
    print("LMDB Database Tree Visualization")
    print("================================")
...
local function show_help()
    ...
    print("=======================")
    print()
    print("A tool for visualizing LMDB database structure as hierarchical trees.")
    print()
    print("Usage: mdb_dump /path/to/lmdb | lmdb-tree-viewer [OPTIONS]")
    print()
    print("OPTIONS:")
    print("  --summary, -s    Show condensed structure summary")
    print("  --parser, -p     Show full tree visualization (default)")
    print("  --help, -h       Show this help message")
    print("  --version, -v    Show version information")
    print()
    print("Examples:")
    print("  mdb_dump /var/lib/myapp/data.lmdb | lmdb-tree-viewer")
    print("  mdb_dump /var/lib/myapp/data.lmdb | lmdb-tree-viewer --summary")
    print("  mdb_dump /var/lib/myapp/data.lmdb | lmdb-tree-viewer -s")
    print()
    print("Note: This tool requires the 'mdb_dump' utility from the LMDB tools package.")
    print("Install with: brew install lmdb (macOS) or apt-get install lmdb-utils (Ubuntu)")
end
...
local function main(args)
    ...
    if args and #args > 0 then
        ...
        if arg1 == "--help" or arg1 == "-h" then
            show_help()
            return
        elseif arg1 == "--version" or arg1 == "-v" then
            show_version()
            return
    ...
    if #keys == 0 then
        ...
        print("mdb_dump /path/to/lmdb | lmdb-tree-viewer")
        print()
        print("If you don't have mdb_dump installed:")
        print("  macOS: brew install lmdb")
        print("  Ubuntu/Debian: apt-get install lmdb-utils")
        print("  CentOS/RHEL: yum install lmdb-utils")
        return
    end
